...
|-------|-------------------------------|---------------------|
| **Engagement phase** | Look for phase signals: "starting", "designing", "reviewing", "implementing", "change request" | *"Where are you in the engagement? (A) Starting — defining scope and architecture vision (B) Designing — developing business, data, application or technology architectures (C) Reviewing — validating or approving existing artifacts (D) Governing — overseeing implementation against the architecture (E) Managing change — handling a change request to a deployed architecture"* |
